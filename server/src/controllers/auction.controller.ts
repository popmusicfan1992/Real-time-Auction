  ...
  try {
    ...
    if (activeCount === 0) {
      ...
      for (const auction of auctions) {
        ...
        if (
          auction.title.includes("Porsche") ||
          ...
          auction.title.includes("Bordeaux") ||
        ...
        await prisma.$transaction([
          ...
          prisma.depositHold.deleteMany({ where: { auctionId: auction.id } }),
          prisma.reminder.deleteMany({ where: { auctionId: auction.id } }),
          prisma.chatMessage.deleteMany({ where: { auctionId: auction.id } }),
